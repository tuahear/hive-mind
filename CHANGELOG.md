...
### Fixed
- **Codex hook config now enters through a native launcher instead of `bash ...` in `hooks.json`.** `setup.sh` builds a small `hivemind-hook` binary and the Codex adapter renders `hooks.json` against that single executable, which then shells into the existing bash scripts internally. This keeps the installed hook command surface down to one native entrypoint instead of a fragile direct-bash invocation.
