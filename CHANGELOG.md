...
### Fixed
- **Codex hooks on Windows now enter through a native launcher instead of `bash ...` in `hooks.json`.** `setup.sh` builds a small `hivemind-hook` binary and the Codex adapter renders `hooks.json` against that single executable, which then shells into the existing bash scripts internally. This removes the PowerShell/WSL path and quoting failures that were still showing up on some Windows machines.
